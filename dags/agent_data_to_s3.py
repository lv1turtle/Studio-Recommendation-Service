...
def download_agent_data(download_path):
    ...
    with webdriver.Remote(f"{remote_webdriver}:4444/wd/hub", options=options) as driver:
        ...
        download_button = driver.find_elements(By.CLASS_NAME, "bt.ico.down.bg.primary")[
            0
        ]
...
def transform_columns(csv_file):
    ...
    df = df[
        ["등록번호", "brkr_nm_encpt", "중개업자종별코드", "직위구분코드", "자격증번호"]
    ]
    df.rename(
        columns={
            "등록번호": "registration_number",
            "brkr_nm_encpt": "agent_name",
            "중개업자종별코드": "agent_code",
            "직위구분코드": "position_code",
            "자격증번호": "certificate_number",
        },
        inplace=True,
    )
