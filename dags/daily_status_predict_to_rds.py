...
def preprocessing_train_data(**context):
    key = context["task_instance"].xcom_pull(
        key="return_value", task_ids="fetch_transform_train_data"
    )
...
def train_ml(**context):
    key = context["task_instance"].xcom_pull(
        key="return_value", task_ids="preprocessing_train_data"
    )
...
def predict_status(**context):
    df_key = context["task_instance"].xcom_pull(
        key="return_value", task_ids="fetch_preprocessed_property_from_rds"
    )
    df = ml_pipeline.read_csv_and_remove(df_key, ml_pipeline.DEFAULT_DIR)

    model_key = context["task_instance"].xcom_pull(
        key="return_value", task_ids="train_ml"
    )
...
def update_predicted_status_in_rds(schema, table, **context):
    key = context["task_instance"].xcom_pull(
        key="return_value", task_ids="predict_status"
    )
    df = ml_pipeline.read_csv_and_remove(key, ml_pipeline.DEFAULT_DIR)
...
with DAG(
    ...
    preprocessing_train_data = PythonOperator(
        task_id="preprocessing_train_data", python_callable=preprocessing_train_data
    )

    train_ml = PythonOperator(task_id="train_ml", python_callable=train_ml)

    insert_accuracy_to_redshift = PythonOperator(
        task_id="insert_accuracy_to_redshift",
        python_callable=insert_accuracy_to_redshift,
        op_kwargs={"schema": "analytics", "table": "model_accuracy"},
    )
    ...
    fetch_preprocessed_property_from_rds = PythonOperator(
        ...
        op_kwargs={"schema": "production", "table": "property"},
    )

    predict_status = PythonOperator(
        task_id="predict_status", python_callable=predict_status
    )
    ...
    (
        fetch_transform_train_data
        >> preprocessing_train_data
        >> train_ml
        >> insert_accuracy_to_redshift
        >> fetch_preprocessed_property_from_rds
        >> predict_status
        >> update_predicted_status_in_rds
    )
