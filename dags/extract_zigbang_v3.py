...
CATEGORY_GROUP = {
    "대형마트": {"code": "MT1", "radius": 1712, "eng": "market"},
    "편의점": {"code": "CS2", "radius": 629, "eng": "store"},
    "지하철역": {"code": "SW8", "radius": 1058, "eng": "subway"},
    "음식점": {"code": "FD6", "radius": 500, "eng": "restaurant"},
    "카페": {"code": "CE7", "radius": 987, "eng": "cafe"},
    "병원": {"code": "HP8", "radius": 946, "eng": "hospital"},
}
    ...
    if response.status_code == 200:
        try:
            ...
            if item_data["status"] == "open":
                ...
                room_data["address"] = (
                    item_data["jibunAddress"]
                    if "jibunAddress" in item_data.keys()
                    else item_data["addressOrigin"]["localText"]
                )
                room_data["property_link"] = (
                    "https://sp.zigbang.com/share/"
                    + room_type_to_eng[item_data["serviceType"]]
                    + "/"
                    + str(item_data["itemId"])
                )
                ...
                room_data["image_link"] = (
                    item_data["imageThumbnail"] + "?w=400&h=300&q=70&a=1"
                )
            ...
            if item_data["status"] == "open":
                ...
                room_data.update(
                    extract_nearest_all_facilities_info(
                        room_data["longitude"], room_data["latitude"]
                    )
                )
...
def extract_nearest_all_facilities_info(lng, lat) -> dict:
    ...
    for category in CATEGORY_GROUP.keys():
        params = {
            ...
            "x": lng,
            "y": lat,
            "radius": CATEGORY_GROUP[category]["radius"],
            "CATEGORY_GROUP_code": CATEGORY_GROUP[category]["code"],
        }
        ...
        try:
            ...
            data[f"nearest_" + CATEGORY_GROUP[category]["eng"] + "_distance"] = (
                nearest_distance
            )
...
def extend_facilities_info(new_info_data):
    for i, item in enumerate(new_info_data):
        facilities = extract_nearest_all_facilities_info(
