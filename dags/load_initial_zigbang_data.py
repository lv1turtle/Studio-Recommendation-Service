...
def extract_room_infos_1(**context):
    ...
    data.extend(extract_zigbang_v3.extract_room_info_include_facilities(room_ids[last_fetched_index:last_fetched_index+DAILY_FETCH_LIMIT]))
    
    context['ti'].xcom_push(key='last_fetched_index', value=last_fetched_index+DAILY_FETCH_LIMIT)
...
def extract_room_infos_2(**context):
    room_ids = context['ti'].xcom_pull(task_ids='fetch_room_ids', key='room_ids')
    last_fetched_index = context['ti'].xcom_pull(task_ids='extract_room_infos_1', key='last_fetched_index')
...
def extract_room_infos_3(**context):
    room_ids = context['ti'].xcom_pull(task_ids='fetch_room_ids', key='room_ids')
    last_fetched_index = context['ti'].xcom_pull(task_ids='extract_room_infos_2', key='last_fetched_index')
...
with DAG('load_initial_zigbang_data',
    ...
    wait_until_midnight_1 = TimeDeltaSensor(
        ...
        delta=timedelta(hours=12, minutes=0) - timedelta(seconds=datetime.now().second,
...
fetch_room_ids >> extract_room_infos_1 >> wait_until_midnight_1 >> extract_room_infos_2 >> wait_until_midnight_2 >> extract_room_infos_3 >> \
load_to_s3 >> clear_data >> load_zigbang_init_data_to_redshift_from_s3